...
def create_region_template(rotation, guidestar_dra, guidestar_ddec, guidestar_sl, guidestar_sw):
  ...
  poly_x = [0.02714,  0.02712,  0.02593,  0.02462, 0.02412, 0.02422, 0.02429, #Default x values for points in FOV polygon
    -0.00265, -0.02095, -0.02338, -0.02403, -0.02681, -0.02677, -0.02518,
    -0.02256, -0.01865, -0.01333, 0.0158,  0.0236]
  poly_y =  [-0.00825096, -0.00809768, -0.00462737,  -0.00309165, 0.00030273, 0.00061059,  0.00618683, #Default y values for points in FOV polygon
  ...
  rotMatrix = array([[cos(rad_rot), -sin(rad_rot)], [sin(rad_rot),  cos(rad_rot)]]) #Set up rotation matrix for polygon
  ...
  poly_xy = poly_xy+str(poly_x[n-1])+', '+str(poly_y[n-1])+')'  #Give final point the special format it needs
  ...
  output.append('global color=green dashlist=8 3 width=1 font="helvetica 10 normal roman" select=1 highlite=1 dash=0 fixed=0 edit=1 move=1 delete=1 include=1 source=1') #Set up color, fonts, and stuff
  output.append('wcs0;fk5') #Set coordinate system
  output.append('# compass(0,0,72") compass=fk5 {N} {E} 1 1 font="helvetica 12 bold roman" color=blue fixed=0') #Set compass (North and East arrows)
  ...
  if abs(guidestar_dra) > 0. or abs(guidestar_ddec) > 0.: #show guidestar if it exists
    output.append('point('+str(guidestar_dra)+','+str(guidestar_ddec)+') # point=circle font="helvetica 12 bold roman" color=yellow text={Offslit guide star [sl: '+"%5.2f" % gstar_sl+', sw:'+"%5.2f" % gstar_sw+']}')
...
from scipy.interpolate import InterpolatedUnivariateSpline #Import interpolation for interpolating telescope HA & Dec. observing limits
...
#      LST = HA + RA
read_limits_dec, read_limits_HA_min, read_limits_HA_max = loadtxt('observing_limit_table.txt', dtype='f4,f4,f4', unpack=True) #Read in text file giving HA limits to the 2.7m scope
...
LST_min = longitude(obj_coords.ra.hour() + HA_min, units='hour') #Calculate min and max Local Siderial time: LST = RA - HA
...
if show_finder_chart == 'y':
  ...
  if finder_chart_fits == '': #Use 2MASS K-band if no user specified fits file is found
    ...
    ds9.set('2mass size ' + str(img_size) + ' ' + str(img_size) + ' arcmin') #Set size of image (weird issues here, only strips extracted)
    if obj_choice == '2': #If user specifies object name
      ...
      ds9.set('2mass coord '+str(obj_coords.ra.deg())+' '+str(obj_coords.dec.deg())+' degrees') #Retrieve 2MASS image
  ...
  if gstar_choice != '0' and gstar_choice !='4': #If single guide star actually put in by user calculate the following parameters...
    gstar_dra_arcsec = ra_seperation(obj_coords, gstar_coords, units='arcsec') #position of guide star from object in arcseconds
    gstar_ddec_arcsec = dec_seperation(obj_coords, gstar_coords, units='arcsec') #position of guide star from object in arcseconds
    ...
    gstar_dx = ( -gstar_dra_arcsec*cos(radians(PA-45.0)) + gstar_ddec_arcsec*sin(radians(PA-45.0)) ) / plate_scale #guide star position in pixels in the SVC display
    gstar_dy = ( gstar_dra_arcsec*sin(radians(PA-45.0)) + gstar_ddec_arcsec*cos(radians(PA-45.0)) ) / plate_scale #guide star position in pixels in the SVC display
    gstar_sl =  -gstar_dra_arcsec*cos(radians(PA-90.0)) + gstar_ddec_arcsec*sin(radians(PA-90.0)) #guide star position relative to slit in arcseconds
    gstar_sw =   gstar_dra_arcsec*sin(radians(PA-90.0)) + gstar_ddec_arcsec*cos(radians(PA-90.0)) #guide star position relative to slit in arcseconds
  
  create_region_template(delta_PA, gstar_dra_deg, gstar_ddec_deg, gstar_sl, gstar_sw) #Make region template file rotated and the specified PA
  ds9.set('regions template IGRINS_svc_generated.tpl at '+ obj_coords.showcoords() + ' fk5' ) #Read in regions template file
  ds9.set('regions select all')
  ds9.set('regions group FOV new')
  ds9.set('regions select none') #Deslect regions when finished 
  ds9.set('align yes') #Set north to be up and east to be left, before rotating, for weird fits files that open at odd angles
  ...
  if gstar_choice == '4': #If user specifies code to find guide stars automatically
    ...
    ds9.set("catalog filter '$RAJ2000>="+str(obj_coords.ra.deg()-gstar_ra_limit)+"&&$RAJ2000<="+str(obj_coords.ra.deg()+gstar_ra_limit) \
      +"&&$DEJ2000>="+str(obj_coords.dec.deg()-gstar_dec_limit) + "&&$DEJ2000<=" + str(obj_coords.dec.deg()+gstar_dec_limit) \
    ...
    if len(lines) > 1:
      open('tmp.dat', 'w').writelines(lines[0:n_gstars+1]) #Truncate by maximum number of guide stars and save catalog list
	...
	found_gstar_dra_arcsec = ra_seperation(obj_coords, gstar_coords, units='arcsec') #position of guide star from object in arcseconds
	found_gstar_ddec_arcsec = dec_seperation(obj_coords, gstar_coords, units='arcsec') #position of guide star from object in arcseconds
	gstar_dx = ( -found_gstar_dra_arcsec*cos(radians(PA-45.0)) + found_gstar_ddec_arcsec*sin(radians(PA-45.0)) ) / plate_scale  #guide star position in pixels in the SVC display
	gstar_dy = ( found_gstar_dra_arcsec*sin(radians(PA-45.0)) + found_gstar_ddec_arcsec*cos(radians(PA-45.0)) ) / plate_scale  #guide star position in pixels in the SVC display
	gstar_sl =  -found_gstar_dra_arcsec*cos(radians(PA-90.0)) + found_gstar_ddec_arcsec*sin(radians(PA-90.0)) #guide star position relative to slit in arcseconds
	gstar_sw =   found_gstar_dra_arcsec*sin(radians(PA-90.0)) + found_gstar_ddec_arcsec*cos(radians(PA-90.0))#guide star position relative to slit in arcseconds
	command_line_output.append("%7.2f" % gmag[i] +'\t'+ "%7.2f" % gstar_sl +'\t'+  "%7.2f" % gstar_sw+'\t\t'+gstar_coords.showcoords()) #Save info on found guide stars to the command line
	ds9.draw('fk5; point('+str(gra[i])+','+str(gdec[i])+') # point=cross font={helvetica 9 bold roman} color=yellow text={[K: ' \
	  +str(gmag[i])+'; SL: '+"%5.2f" % gstar_sl+'; SW: '+"%5.2f" % gstar_sw+']}') #Put pointer regions to guide stars in DS9
    else:
      ds9.set('catalog clear') #Clear 2MASS catalog
      ds9.set('catalog close') #Close 2MASS catalog window
