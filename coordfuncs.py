...
def deg2sex(input, precision=1, sign='', units='dms'):
    ...
    if units == 'dms' or units == 'hms':  #return everything in format "xx:xx:xx.xx"
        fmt = "%02d:%02d:%02d.%0" + str(
            int(precision)) + "d"  #Set up formatting for string that will hold the sexagesimal output
...
def angular_seperation(obj1, obj2, units='deg'):
    ddeg = degrees(
        arccos(obj1.dec.sin() * obj2.dec.sin() + obj1.dec.cos() * obj2.dec.cos() * cos(obj1.ra.rad() - obj2.ra.rad())))
...
def alt(tar, loc, LST):
    ...
    return degrees(arcsin(
        sin_lat * sin_dec + cos_lat * cos_dec * cos_HA))  #Equation from http://koti.mbnet.fi/jukaukor/Star_altitude_azimuth.pdf
...
def az(tar, loc, LST):
    ...
    return degrees(arccos((sin_dec - sin_lat * sin_alt) / (
    cos_lat * cos_alt)))  #Equation from http://koti.mbnet.fi/jukaukor/Star_altitude_azimuth.pdf
